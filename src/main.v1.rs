...
mod reads;
use reads::{SimReads, create_bam_header, reverse_complement_seq};

const COUNTER_BITS: usize = 10;
const CFSIM_MAGIC: &[u8] = "CF01\u{1}".as_bytes();
// const UNALIGNED_ERROR_RATE: f64 = 0.7;
...
impl KmerDist {
    ...
    fn sample<'a>(
        ...
        match error_type {
            ...
            Self::ALL => {
                ...
                self.keys.get(idx).map(|x| &errors[*x as usize])
            }
    ...
    fn update_index_weight_with_temperature(
        ...
        //TODO use raw_weights buffer
        let new_weights: Vec<u32> = dist
            .weights()
            .enumerate()
            .map(|(p, w)| {
                if p == index {
                    ((w as f64 * temperature) as u32).clamp(1, u32::MAX / 2)
                } else {
                    w
                }
            })
            .collect();
        WeightedIndex::new(&new_weights).unwrap()
    }
}
...
fn build_kmer_identity_distribution(
    ...
    let bin_num = ((kmer_max_error / KMER_ERROR_BIN).ceil() as usize) + 1;

    let mut binned = vec![HashMap::default(); bin_num];

    for &(kmer_error, identity) in errors_rate {
        let bin_idx = (kmer_error / KMER_ERROR_BIN).floor() as usize;
...
fn read2kmer(
    ...
    for cig in cigar.iter() {
        let code = match cig {
            Cigar::Equal(len) => Some((0, *len)),   // '=' -> 0
            Cigar::Diff(len)  => Some((3, *len)),   // 'X' -> 3
            Cigar::Del(len)   => Some((1, *len)),   // 'D' -> 1
            Cigar::Ins(len)   => Some((2, *len)),   // 'I' -> 2
            _ => None,  // skip others like Pad, RefSkip
        };

        if let Some((val, len)) = code {
            ops.extend(std::iter::repeat(val).take(len as usize));
        }
    }


    let identity = ops.iter().filter(|x| **x == 0).count() as f32 / ops.len() as f32;
    if let Some((
        (ref_left_offset, ref_right_offset),
        (read_left_offset, _read_right_offset),
        (alignment_left_offset, alignment_right_offset),
...
fn main() {
    ...
    if opt.cmd == "rhq" {
        ...
        let is_high_quality = *global_has_error < 0.01;
        ...
        let kmer_identitys = &build_kmer_identity_distribution(errors_rate);
        ...
        thread::scope(|work| {
            ...
            for i in 0..opt.thread as u64 {
                ...
                work.spawn(move || {
                    ...
                    let (mut seq_buffer, mut cigar_buffer) = (Vec::new(), Vec::new());
                    let (mut bam_index, mut bam_buffer) = (1, String::new());
                    let mut sim_reads = SimReads::new();
                    for (tid, ref_seq) in ref_seqs.iter().map(|x| &x.1).enumerate() {
                        let ref_rev_seq = &reverse_complement_seq(ref_seq);
                        ...
                        while sim_bases <= total_bases {
                            ...
                            let mut start_pos = pos_dist.sample(&mut rng);
                            ...
                            seq_buffer.clear();
                            cigar_buffer.clear();
                            let seq = if is_positive { ref_seq } else { ref_rev_seq };
                            while seq[start_pos] == b'N' {
                                //skip N bases
                                start_pos += 1;
                            }
                            ...
                            if !opt.disable_unaligned
                                && end_pos + (unaligned_left_len as usize) < seq.len()
                            {
                            ...
                            if end_pos + (aligned_len as usize) < seq.len() {
                            ...
                            if !opt.disable_unaligned
                                && end_pos + (unaligned_right_len as usize) < seq.len()
                            {
                                end_pos += sim_unaligned_seq(
                                    &mut rng,
            ...
            let result1 = work.spawn(move || {
                ...
                while let Some(ret) = bam.read(&mut record) {
                    ...
                    let (read_start, read_end) =
                        get_read_aligned_range(&cigar, record.seq_len() as u32);
                    ...
                    let result = thread::scope(|kmer| {
                        ...
                        for i in 0..opt.thread {
                            ...
                            let handle = kmer.spawn(move || {
                                ...
                                for (p, &(read_start, read_end, ref_id, ref_start, ref_end, std, ref cigar)) in
                                    seqs_info[s..e].iter().enumerate()
                                {
